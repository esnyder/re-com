(ns re-demo.simple-v-table-editable
  (:require-macros
  ...
  (:require
    ...
    [re-com.core           :refer [label p slider hyperlink-href h-box v-box gap simple-v-table checkbox button md-icon-button]]
...
(defn delete-icon
  []
  (md-icon-button ;:md-icon-name "zmdi-delete"
                  :src (src-coordinates)))
...
(defn demo
  ...
  (let [sales-rows          (reagent/atom (generate-sales-rows 100))
        ...
        spacing             (px 12)
        spacing7           (px 7)
        email-row-label-fn  (fn [row] [hyperlink-href :attr {:id (str (:id row) ".email")} :label (:email row) :href (str "mailto:" (:email row))])
    ...
    (fn []
      [v-box
       :src      (src-coordinates)
       :children [[title3 "Editable Table Demo"]
                  [gap :size (px 5)]
                  [v-box
                   :src      (src-coordinates)
                   :gap      (px 10)
                   :children [[v-box
                               :src      (src-coordinates)
                               :width    "450px"
                               :children [[md-icon-button :md-icon-name "zmdi-delete"]
                                          [:p "This demo shows the primary use case - a table displaying entities in rows, and their attributes in columns. See it below in the blue box."]
                                          ...
                                          [v-box
                                           ...
                                           :children [[label
                                                      ...
                                                      [simple-v-table
                                                       ...
                                                       ;; ===== Columns
                                                       :columns                   [{:id nil     :header-label ""       :row-label-fn delete-row-label-fn   :width 70  :align "center" :vertical-align "middle"}
                                                                                   {:id :id     :header-label "Code"   :row-label-fn :id                   :width 70  :align "center" :vertical-align "middle"}
                                                                                   {:id :region :header-label "Region" :row-label-fn :region               :width 100 :align "left"   :vertical-align "middle"
                                                                                    :on-change  edit-sales-rows!
                                                                                    :disabled-fn (fn [row-id col-id]
                                                                                                   ;;(println "(disabled? " row-id col-id ") -> false")
                                                                                                   false)
                                                                                    }
                                                                                   {:id :name   :header-label "Name"   :row-label-fn :person               :width 100 :align "left"   :vertical-align "middle" :on-change edit-sales-rows! :sort-by {}}
